...
def preprocess_geoboundaries(input_geojson, input_GBR_regions, output_geojson):
    ...
    # Normal GBR (Great Britain) data may be too large to process, its simplified version is used instead
    if Path(input_geojson).stem[: 3] == "GBR":
        national_regions = gpd.read_file(input_GBR_regions).to_crs(3035)
    else:
        national_regions = gpd.read_file(input_geojson).to_crs(3035)
...
if __name__ == "__main__":
    preprocess_geoboundaries(
        ...
        input_GBR_regions = snakemake.input[1],
